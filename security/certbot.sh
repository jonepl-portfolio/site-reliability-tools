...
is_domain_in_ca_certificate() {
    local domain=$1
    log_message "Checking if domain $domain is in the CA certificate"
    if openssl x509 -in "$SSL_PATH/fullchain.pem" -text -noout | grep -q "DNS:$domain"; then
        return 0  # Found
    else
        return 1  # Not found
    fi
...
create_certificate_authority_certificate() {
    ...
    if [ ! -d "$SSL_PATH" ] || \ 
        ...
        log_message "INFO" "Certificate for domains are already valid and not expiring soon."
    fi
}
...
while :; do
    if [ $IS_LOCAL -eq 0 ]; then
        log_message "INFO" "Attempting to renew Certbot certification ..."
        certbot renew
    fi

    log_message "INFO" "Sleeping for 12 hours..."
    sleep 12h
done
